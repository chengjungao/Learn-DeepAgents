# 学习笔记：从 Harness 认知到自定义工具

> 课程：Datawhale《Deep Agents 实战》
> —— ch01《从 Agent Framework 到 Agent Harness — Deep Agents 的诞生逻辑》（Lec 01 + Lec 02）
> —— ch02《快速上手 — 5 分钟构建你的第一个 Deep Agent》（Lec 03）
> 环境：原生 Windows（无 WSL）、Python 3.13.14、deepagents 0.7.13
> 模型：智谱 GLM-5.2（OpenAI 兼容接口）
> 日期：2026-09-17
> 项目路径：`D:\Works\DeepAgents学习\research_deepagent`
> 上一篇：`Task1-AgentSeek环境搭建与踩坑.md`（pre01 + pre02）

---

## 〇、这轮做了什么

ch02 的实验要求是「改提示词和工具，跑通第一个 Deep Agent」。所以这轮不只读，**真的往 research 项目里加了工具并跑通**。

| 环节 | 结果 |
|---|---|
| 新增工具 | **2 个**，追加进项目原有的 `tools.py`（没另起模块） |
| 注册位置 | 主图的 `research-agent` 子代理 → 声明工具从 2 个变 **4 个** |
| 工具直调验证 | 命中 / 越界拒绝 / 空查询 / 无命中，**4 类分支全过** |
| 项目级 e2e | **16.5s**，模型自主调本地工具 + `think_tool`，产出带「文件 + 行号」的引用 |
| 系统级 e2e | 前端跑通全链路，**并暴露一个此前没预料到的设计缺陷**（见第 4 节坑 4，这是本轮最值钱的发现） |
| 连带修复 | 研究员提示词写「two specific research tools」，实际已是 4 个 —— 同类错配，一并改掉 |

一句话概括新增工具的定位：
**Tavily 管开放网络，新增的两个管「你自己的笔记和项目文档」—— 免费、离线、可引用。**

---

## 一、课程精华（只留能直接用的部分）

### 1.1 三层架构：这是本轮最有复用价值的认知

ch01 不写代码，只回答一个问题：**已经有那么多 Agent 框架了，Deep Agents 为什么还要存在？**

答案是把 Agent 开发切成三层，自底向上层层构建：
...
| 层次 | 代表 | 解决什么问题 | 同层其他选手 |
|---|---|---|---|
| **Runtime** | LangGraph | Agent 怎么**可靠地运行** | Temporal、Inngest |
...
课程那个比方很好记：Runtime = 工作台和电源，Framework = 锤子锯子，Harness = 一个装好了的工具间。

**这三层不是替代关系。判断一个 Agent 问题该在哪层解，用这个框架会快很多。**

### 1.2 为什么需要 Harness

看那些真能完成复杂任务的 Agent（Claude Code、Manus、Cursor），核心能力惊人地相似：文件系统操作、任务规划、子任务委派、上下文管理。

**这些共性不是巧合** —— 任务足够复杂时，它们就是必需品。Harness 的价值就是把共性固化，不让你每次从头造。

### 1.3 Context Engineering：虚拟文件系统是核心

传统做法（Prompt Stuffing）把 20 个文件的完整内容一次性塞进上下文，三个致命问题：窗口溢出、**注意力稀释**（信息越多，模型对关键信息的关注越低）、不可扩展。

Deep Agents 的做法是引入**虚拟文件系统**：要读才 `read_file`，要记就 `write_file`，大文件用 `offset` / `limit` 只读需要的部分。上下文里**只留当前步骤真正需要的信息**。
...
> **一句话**：Context Engineering 不是「把所有信息喂给 LLM」，而是**为 LLM 建一套高效获取和管理信息的基础设施**。
>
> ⚠️ **注意这张表的第二行和第一行的区别** —— 第 4 节坑 4 整个事故都源于这两行没分清。
...
| **Docstring** | 告诉 Agent 这个工具**什么时候用** | Agent 不知道该不该调它 |
| **默认值** | 标记可选参数 | Agent 被迫填所有参数，出错率上升 |

一句分工：**类型标注决定输入形态，docstring 决定调用时机，默认值减少必填项。**
...
选型口诀：要模型灵活性 + 跨会话记忆 → Deep Agents；团队全用 Claude → Claude Agent SDK；团队全用 OpenAI → Codex SDK。

> 课程的「技术全景图」页面自己加了 v0.7 提醒。我把课程描述和本机实测对了一遍，
> 唯一有出入的是文件工具数（课程说 7 个，实测 8 个）—— 差的那个 `execute` 属执行类工具，
> 不算「文件操作」，**结论一致，不必当成版本差异**。
...
选工具不是随手挑，是照着项目里**现成的缺口**定的：

| 观察 | 推论 |
|---|---|
| 研究子代理只有 `tavily_search` + `think_tool` | 唯一的检索源是**开放网络**，且联网、按次计费、要 Key |
| deepagents 默认 backend 是 `StateBackend`（`graph.py:637`）| 内置 `read_file` / `grep` / `glob` 读的是**虚拟 FS**，看不到硬盘上的真实文件 |
| 项目自己的 `Learn-Notes/`、`README.zh-CN.md` 已经是可引用的知识 | Agent 却读不到它们 |

**三点合起来就是一个明确缺口**：Agent 需要一个「读用户自己资料」的通道。

而且这两个工具正好和产线方向对得上 —— 现在是词面匹配的最小形态，
将来换成 bge-m3 向量召回 + Milvus + reranker，**接口形状不变（query 进 → 带出处与片段的结果出），Agent 侧无需改动**。

### 2.2 两个工具与四个设计取舍

工具直接追加进项目原有的 `src/research_deepagent/tools.py`（那本来就是「研究工具」的归属地）。

| 工具 | 模型可见参数 | 作用 |
|---|---|---|
| `search_local_notes` | `query` | 在本地笔记根目录做关键词检索，返回**文件路径 + 行号 + 命中行** |
...
| **写清「何时用」** | docstring 首句点明「本地可能已有答案时才用，否则退回 tavily」 | 这正是三要素里 docstring 的职责 —— 两工具并用时，模型靠这段决定顺序 |
...
> **为什么不给主 Agent？** 主 Agent 的提示词有一条硬约束：
> `ALWAYS use sub-agents for research, never conduct research yourself`。
> 查资料属于「research」，就该待在子代理里。给它配上，等于在架构上开后门。

**⚠️ 这个决定在第 4 节被证明是不完整的 —— 但问题不在「该不该给」，而在「验证通道」缺席。先看证据。**
...
> 顺带一个**自指现象**：这个工具把笔记目录本身当检索源，所以「扫描 N 个文件」这个数**会随笔记增多而变**。
> 这类「索引自身」的循环在真实检索系统里也要留意 —— 索引会污染被测对象。

**证据 ②｜项目级 e2e（跑项目里声明的那个子代理）**
...
1. **模型自己换了 4 个中英文变体去搜** —— 说明它把「关键词检索」的语义理解对了。换用词面工具时会主动换词，这是向量检索不需要但词面工具必须具备的能力。
2. **它没调 `tavily_search`** —— 本地资料足够时自动收敛，一次外部额度都没花。
3. **引用格式直接落到文件 + 行号**，`### Sources` 能直接组织，不需要额外提示。
...
> 顺带发现：`create_agent` 只在**运行时**才把工具绑给模型（构建图时 `bind_tools` 调用次数为 0）。
> 所以「图编译成功」≠「工具已绑定」，要验证得真的跑一次 —— 这也是我做证据 ②的原因。

### 2.5 一处连带修复：提示词又在枚举工具清单
...
**现在有 4 个工具，提示词却说「你有两个」。** 这和 Task1 的 `write_todos` 是**同一类错误** —— prompt 写死了工具清单，工具集一变就成了死指令。
...
> **教训**：课程 ch02 说得对 —— v0.7 时代提示词「只需要说明研究目标、证据要求和输出边界」，工具接口由 **Schema** 表达。
> 我第一反应是「去改提示词补上清单」，但更对的做法是**不去枚举工具**，只写策略。
> **枚举清单 = 给自己埋一个会过期的字段。**
...
**① Task1 装的循环护栏真的进了图。** 图节点序列（也是 trace 的节点序列）为：
...
`ModelCallLimitMiddleware.before_model` / `after_model` 两个节点清楚可见 —— **上一次改的护栏是可观测的，不是"加了但看不出有没有生效"**。
同时图里**没有** `TodoListMiddleware` 节点，也再次印证了 Task1 的结论：默认模板没启用任务规划。

**② Studio 能看 trace ≠ 数据上了 LangSmith 云端。** 这一点很容易误解，值得说清楚：

- `lifecycle.toml` 里 `services.langgraph.links.studio` 写的是
  `https://smith.langchain.com/studio/?baseUrl=http://127.0.0.1:2024` ——
  Studio 是**直连本地 agent server** 渲染的，看到的是本地 server 自己的运行记录。
- 而 `.env` 里 `LANGSMITH_TRACING=false`、`LANGSMITH_API_KEY` 为空 ——
  **云端上传通道并没有打开**。
- 页面顶部那条告警也印证了这种"半通"状态：
  *"Studio tracing requires langgraph-api 0.11.1 or later with session-name tracing enabled. Your server reports version 0.2.3."*
  本地跑的是 `agentseek-api 0.2.3`，低于 Studio 增强追踪所要求的版本。

> **结论**：要真正把 trace 落到云端做跨机器、跨时间的对比，还是得开 `LANGSMITH_TRACING` 并配 Key。
> 当前这套只够「当场看一次」。
...
### 坑 1：`load_dotenv()` 相对「当前工作目录」找 `.env`，不是相对脚本

脚本本来放在项目目录里跑得好好的，挪到工作区根之后**直接挂了**：

```
openai.OpenAIError: Missing credentials. Please pass an `api_key`, ...
```

**根因**：`load_dotenv()` 默认从 cwd 向上找 `.env`。cwd 一换就找不到 —— 而报错看起来像「Key 没配」，很容易往错的方向查。

**修法**：显式指向项目 `.env`，让脚本在任何 cwd 下都能跑：
...
> 这和 Task1 的坑 5（内联环境变量静默覆盖 `.env`）是**一对**：一个从 shell 进来，一个从文件读进来，
> 都栽在「你到底加载了哪个配置」上。
> **`python-dotenv` 相关的怪问题，第一反应应该是「它读了哪个文件、优先级是什么」。**

### 坑 2：扫描本地目录时，构建产物会混进结果

第一版工具扫出 22 个文件，里面混着 `src/research_deepagent.egg-info/*.txt`（构建元数据）。这些不是笔记，混进来只会稀释检索质量。

**修法**：目录过滤加两条规则 —— 名字在 `SKIP_DIR_NAMES` 里、或以 `.` 开头、或以 `.egg-info` 结尾，全部跳过。修完扫描数 **22 → 18**。

> **可推广**：任何「扫本地目录做检索」的工具，**忽略规则和匹配规则一样重要**。
> 真实目录里永远有 `.venv` / `node_modules` / `__pycache__` / 构建产物，不显式排除，信噪比会低到没法用。

### 坑 3：探测工具 schema 时拿错了属性，差点写出错误结论

这个坑是**我自己挖的**，值得单独记 —— 因为它的代价是一条写错的结论。
...
但 `args_schema` 是**全量参数**的签名，真正发给模型的是 `tool_call_schema`：
...
> **教训**：验证「模型看到了什么」要用 `tool_call_schema`；`args_schema` 是给调用方看的全量签名。
> 两个属性只差一个词，结论却相反 ——
> **探测类脚本尤其要先确认自己量的是哪个对象，否则量得越准，错得越真。**

顺带明确两种写法的分工：想让模型自己调 → 普通默认值参数（如 `read_local_note.start_line`）；
想锁死由代码控制 → `Annotated[T, InjectedToolArg]`（本项目 `tavily_search` 和 `search_local_notes.max_results` 都选这种）。

### 坑 4 ★：虚拟文件系统和真实磁盘是**两套东西** —— 编排层的「核实」把正确答案判成了编造

**这是本轮最值钱的发现，也是唯一一个"加完工具反而让结果变差"的问题。**

#### 现象

第 3 节的运行里，编排层用 `glob` 去核实子代理引用的本地笔记，4 次全部返回 `No files found`，
于是判定「子代理编造」，把正确的候选人整段排除，最终答案转向了一个同名但无关的项目。

#### 证据链（全部来自本地 checkpoint，逐条可复核）

**① 编排层真的用了 `glob` 去核实**，四个模式分别是：
...
**③ 它的推理过程（`think_tool` 的原文）——** 这段最值得读，因为**每一步推理在它自己的信息世界里都是对的**：
...
#### 根因：一句话 —— `glob` 看的是虚拟 FS，不是磁盘

实测 `StateBackend` 的报错信息把这件事说得比任何文档都清楚：
...
**注意最后那句提示** —— 它明说虚拟 FS 的初始内容是空的，要靠 `invoke(..., files={...})` 预置。
而 `graph.py:637` 的默认 backend 就是 `StateBackend`：

```python
backend = backend if backend is not None else StateBackend()
```

所以：**磁盘上有 19 个 md，虚拟 FS 里 0 个，`glob` 永远返回空。**

#### 最要命的地方：空结果**无法区分**「不存在」和「查错地方了」
...
**能力给在子代理身上，验证通道却留在编排层手里 —— 而那个验证工具天生看不见子代理的证据来源。**
于是「查不到」被自动解读成「不存在」，「不存在」被解读成「编造」。
**这不是模型幻觉，是工具拓扑造成的系统性假阴性。**

#### 三条修法（按代价排序）

| 方案 | 做法 | 代价 | 评价 |
|---|---|---|---|
| **A. 只读通道下放给编排层** | 把 `read_local_note` 也加进 `create_deep_agent(tools=[...])` | 改 1 行 | 最小改动。编排层本来就有 `Verify` 步骤，给它一个**只读、窄作用域**的核实工具是合理的，不算"自己去做研究" |
...
1. **给一个 Agent 新增「读某处」的能力时，必须同时问：谁需要核实这个能力产出的证据？它有对等的通道吗？**
   只补一半，比不补更糟 —— 因为错误会带上"已验证"的伪装。
2. **工具的「空结果」必须能区分语义。** `No files found` 至少应该带上「在哪个 FS 里找的、找了几个根」，
   否则调用方只能靠猜。这和我自己那个 `search_local_notes` 在无命中时输出
   `Scanned N file(s) ... Consider falling back to tavily_search` 是同一个设计原则 ——
   **空结果要自带上下文，不然就是误导。**
3. **「加了工具」不等于「系统变强」**：这次工具本身 4 类分支全过、子代理也答得很好，
   但整体输出质量**反而下降**。评测要看端到端结果，不能只看单元级验证。
...
1. **Harness 的价值是「把共性固化」。** 三层架构这个切分很干净：Runtime 管「怎么可靠跑」，Framework 管「怎么标准化写」，Harness 管「开箱即用什么」。判断一个 Agent 问题该在哪层解，用这个框架会快很多。

2. **Context Engineering 可以直接搬。** 虚拟 FS + 按需读取 + 可插拔后端，和「多 Agent + 独立记忆」是同一个动机的两个切面 —— 都是**别让上下文里堆不该堆的东西**。

3. **工具的 docstring 就是它的 API 文档 —— 但读者是模型。** 三要素里最容易被低估的是 docstring：类型标注决定**输入形态**，docstring 决定**调用时机**。这次两个工具能做到 16.5s、零 Tavily 收敛，靠的就是那句「本地可能已有答案时才用」。

4. **提示词里不要枚举工具清单。** `write_todos`（Task1）和「two specific research tools」（本轮）是同一类 bug。**写策略，不写清单。**

5. **「图编译成功」不等于「工具绑好了」。** `create_agent` 在运行时才 `bind_tools`，构建期调用次数为 0。注册必须**真跑一次**验证。

6. **★ 能力与验证通道必须成对交付。** 见坑 4。只加「读」的能力、不补「核实」的通道，等于给系统装了一个会产出**假阴性**的裁判 —— 它比没有裁判更危险。
...
| 文件 | 变更 | 说明 |
|---|---|---|
| `research_deepagent/src/research_deepagent/tools.py` | **+2 工具** | 追加 `search_local_notes` / `read_local_note`，及辅助函数与配置常量；模块 docstring 补充「本地工具」章节 |
| `research_deepagent/src/research_deepagent/agent.py` | +6 行 | 导入 2 个新工具；`research_sub_agent["tools"]` 从 2 个装到 4 个 |
| `research_deepagent/src/research_deepagent/prompts.py` | 改 1 段 | `<Available Research Tools>` 从「two tools」改为 4 个工具 + 检索策略 |
| `_ch02_demo.py`（工作区根） | 新增 | ch02 实操验证脚本：工具直调 + 最小 Agent e2e |
| `Learn-Notes/image/task2-frontend-run.png` | 新增 | 前端完整运行截图（含坑 4 现场） |
| `Learn-Notes/image/task2-langsmith-studio-trace.png` | 新增 | LangSmith Studio 图结构 + TURN 1 节点序列 |

> 未改动：`langgraph.json`、`.agentseek/lifecycle.toml`、`.env`、前端。
> 约束：`.env` 含真实 Key，**不入库**（已在 `.gitignore` 中排除）。
